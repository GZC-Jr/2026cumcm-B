...
官方模拟器：python rolling939v2.py --robot-id 参赛队号
本地自测：python rolling939v2.py --self-test 10
...
def update_gaussian(target,q,angle):
    ...
    positives=target['stations'];negatives=target.get('negatives',[])
...
def update_unknown_belief(belief,coverage_map,ch):
    ...
    points=coverage_map.centers
    _,upper=radius_interval(points,[],belief['negatives'])
    ...
    prior=BAYES_CONFIG['prior_exists']
    belief['existence']=max(1e-9,prior*evidence/(1-prior+prior*evidence))
    note='negative_observation_moment_projection'
    if weights.sum()<=1e-12:
        weights=coverage_map.remaining[ch-1].astype(float)
        note='grid_boundary_moment_fallback'
    if weights.sum()>0:belief['mean'],belief['covariance']=gaussian_moments(points,weights)
...
def detection_information(belief,coverage_map,q):
    ...
    lower=np.maximum(1000.,np.linalg.norm(points-np.asarray(q),axis=1))
    new=np.maximum(upper-lower,0.)
    inside=np.linalg.norm(points,axis=1)<=1800
    a=float(new[inside].sum()/max(1e-12,old[inside].sum()))
    probability=min(1.,max(0.,belief['existence']*a))
...
def bayes_select(poly,stations,current,target,deadline):
    ...
    begin=time.monotonic();mu=np.asarray(target['mean']);cov=np.asarray(target['covariance'])
    stations=np.asarray(stations);scale=1000.
    variance=math.radians(BAYES_CONFIG['noise_deg'])**2
    triangles,_=mesh(poly,2)
    station_r2=np.sum((triangles[:,:,None,:]-stations[None,None,:,:])**2,axis=3)
    def receive(z):
        d2=np.sum((triangles-z*scale)**2,axis=2)
        upper=np.minimum(np.max(d2-1e6,axis=1),np.min(np.max(d2[:,:,None]-station_r2,axis=1),axis=1))
        return float(upper.max())/1e6
    def separation(z):return float(np.linalg.norm(z*scale-mu)-20)/scale
    def station_gap(z):return float(np.min(np.linalg.norm(z*scale-stations,axis=1))-5)/scale
    def information(z):
    ...
    def feasible(z):return receive(z)<=-1e-10 and separation(z)>=-1e-8 and station_gap(z)>=-1e-8
    ...
    bound=max(1000.,float(np.max(np.linalg.norm(stations-anchor,axis=1))))
    box=[((anchor[k]-bound)/scale,(anchor[k]+bound)/scale) for k in range(2)]
    constraints=[{'type':'ineq','fun':lambda z:-receive(z)-1e-9},
    ...
    for z in starts:
        ...
        if feasible(result.x):pool.append((objective(result.x),result.x))
    ...
    original_dop=depth_weighted_dop(poly,target['stations'][0],original_point)
    ...
    for candidate_value,candidate_z in pool:
        ...
        if primary_loss<=min(cfg['tau'],cfg['max_primary_loss_ratio'],cfg['primary_tie_rel_tol'])+1e-15:
            ...
            dop=depth_weighted_dop(poly,target['stations'][0],p)
    ...
    reception_certificate={'certified':receive_upper<=-1e-4,
...
def mandatory_stop_measurement_score(target,q):
    ...
    # 对凸定位区域，距固定点的最大值在顶点取得；不超过1000米即保证能接收。
    receive_bound=max(np.linalg.norm(np.asarray(v,dtype=float)-q) for v in poly)
    if receive_bound>1000-1e-6:return None
...
def route_aligned_measurement(poly,stations,current,target,next_stop,free_point,free_detail):
    ...
    def information(p):
        ...
        return 0.5*math.log1p(max(0.,float(h@cov@h))/variance)
    def feasible(p):
        return (receive(p)<=-1e-4 and np.linalg.norm(p-mu)>=20-1e-7 and
